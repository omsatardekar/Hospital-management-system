...
export function createMockDb(): MockDb {
  ...
  const patients: Patient[] = Array.from({ length: 140 }).map((_, i) => {
    ...
    return {
      ...
      status: pick(['ACTIVE', 'IN_TREATMENT', 'DISCHARGED', 'ADMITTED', 'BLOCKED'] as const),
      lastVisit: isoDaysAgo(faker.number.int({ min: 0, max: 60 })),
      department: dept,
      assignedDoctorId,
      allergies: Math.random() > 0.7 ? [pick(['Penicillin', 'Peanuts', 'Dust', 'Pollen', 'Latex'])] : [],
      diseases: Math.random() > 0.8 ? [pick(['Diabetes Type 2', 'Hypertension', 'Asthma', 'Arthritis'])] : [],
      medicalTimeline: [
        { at: isoDaysAgo(60), title: 'Registration', note: 'Patient registered in HMS', type: 'GENERAL' },
        { at: isoDaysAgo(25), title: 'Consultation', note: 'Initial diagnosis recorded', type: 'APPOINTMENT' },
        { at: isoDaysAgo(8), title: 'Follow-up', note: 'Medication adjusted', type: 'GENERAL' },
      ],
      reports: [
